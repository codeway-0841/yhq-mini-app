...
2026-09-05 sanasidagi xavfsizlik, ishonchlilik va arxitektura auditi doirasidagi 18 topilma uchun kod o'zgarishlari kiritilgan. Quyidagi status qayta verifikatsiyadan keyin yangilandi: unit, integration, typecheck, build va production dependency audit gate'lari yashil.
...
- **Verification Summary:**
  - **ESLint:** 0 error (4 hook dependency warnings).
...
| **03** | **P1** | `server/db/connection.ts`<br>`server/modules/auth/auth.service.ts` | Neon Serverless da `transactionBestEffort` haqiqiy ACID transaction bermasligi | ✅ **Tuzatildi** | Account linking/adopt rename Neon'da `transactionHttp` ichidagi ikki statement bilan (rename + identity recreate) atomik bajariladi; lokal/CI Postgres'da `transactionBestEffort` haqiqiy transaction ishlatadi. | Backend typecheck; auth focused 25/25; full integration 276/276 |
...
| **05** | **P2** | `server/modules/promo/promo.repository.ts`<br>`server/modules/payments/payment.router.ts` | Promokod max_uses poygasi va pending orderlar hisobga olinmasligi | ✅ **Tuzatildi** | Promo code uchun advisory lock + promo row lock ostida tekshiruv va `payment_orders` INSERT bitta transactionga o'tkazildi. | `tests/unit/server/promo-discount.test.ts` 11/11; `tests/integration/api/promo-concurrency.test.ts` 1/1 |
...
| **15** | **P2** | `tests/integration/api/security-critical.test.ts`<br>`tests/integration/api/tournament-history.test.ts` | Integratsiya testlarida parallel fixture to'qnashuvlari | ✅ **Tuzatildi** | Security fixture canonical `resolveSubject('yhq').dataSourceId` (`traffic_rules_db`) bilan moslashtirildi; topic fixture scope qilindi. Parallel integration suite qayta ishga tushirilganda to'liq o'tdi. | Full integration 36/36 files, 276/276 tests |
...
## 2.1. Qayta verifikatsiya natijalari

- Focused `security-critical.test.ts`: **14/14 o'tdi**; fixture canonical bank ID bilan tuzatildi.
- Focused `explanations.test.ts`: **6/6 o'tdi**.
- Focused `auth.test.ts`: **25/25 o'tdi** (`--testTimeout=60000`); Neon linking endi HTTP transaction orqali bajariladi.
- Focused remediation unit set: **88/88 o'tdi**.
- Full integration run: **36/36 fayl va 276/276 test o'tdi**. Auth phone-link rename va promo concurrency regressiyalari parallel suite ichida ham yashil.
- `npm test`: **173 fayl, 1,165 test o'tdi**.
- E2E parallel run: **22/26**; 4 Android WebView cases route-load timeout bilan yiqildi. Shu 5 ta case `--workers=1` bilan **5/5 o'tdi**, shuning uchun bu run flaky parallel resource contention sifatida qayd qilindi.
- Frontend/backend typecheck, frontend/server/API build o'tdi.
- `npm audit --omit=dev`: **0 vulnerability** qaytardi.
