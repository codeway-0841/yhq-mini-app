...
export default function DonutChart({ correct, total, threshold = 90, hideVerdict = false, passedLabel = "O'tdi", failedLabel = "O'tmadi" }: {
  ...
  return (
    <div className="relative w-28 h-28 mx-auto my-2.5">
      <svg viewBox="0 0 112 112" className="w-full h-full -rotate-90">
        <circle cx={cx} cy={cy} r={r} fill="none" stroke="var(--theme-line)" strokeWidth="10" />
        {shownWrong > 0 && (
          <circle cx={cx} cy={cy} r={r} fill="none" stroke="var(--p-danger)" strokeWidth="10"
            strokeDasharray={`${shownWrong} ${circumference}`}
            strokeDashoffset={-shownArc} />
        )}
        {shownArc > 0 && (
          <circle cx={cx} cy={cy} r={r} fill="none" stroke="var(--p-primary)" strokeWidth="10"
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        ...
        {!hideVerdict && (
          <span className="text-[11.5px] font-semibold mt-0.5 text-psubtle">
