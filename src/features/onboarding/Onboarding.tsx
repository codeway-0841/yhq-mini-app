  ...
  return (
    <Screen>
      ...
      <div className="relative my-3 flex min-h-0 flex-1 items-center justify-center">
        <picture className="flex max-h-full items-center justify-center splash-float-safe">
  ...
  return (
    <Screen>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain pr-1">
        <div className="flex flex-col gap-3 sm:grid sm:grid-cols-2 sm:items-start">
          {GOALS.map((g) => {
            ...
            return (
              <button key={g.id} type="button" onClick={() => setGoal(g.id)} aria-pressed={active}
                className={`flex min-h-16 w-full items-center gap-3.5 rounded-2xl p-3.5 text-left shadow-xs transition-all active:scale-[0.98] ${
                  active ? 'ring-2 ring-pprimary bg-[rgb(var(--p-primary-rgb)/0.10)]' : 'bg-pcard hover:bg-psurface'
                }`}>
