...
function useTimer(onTimeUp, resetKey) {
...
export default function TestPage() {
  ...
  const goTo = useCallback(
    (i) => { if (i >= 0 && i < activeQuestions.length) setCurrent(i) },
  ...
  const getOptionState = useCallback((optId) => {
  ...
  const handleSelect = useCallback((optId) => {
    ...
    setSelectedHistory((prev) => {
      const next = [...prev]; next[current] = optId; return next
    })
    setAnswers((prev) => {
      const next = [...prev]; next[current] = isCorrect ? 'correct' : 'wrong'; return next
    })
    addResult(isCorrect)
  ...
  }, [selected, correctId, current, settings, addResult])

  const buildResults = useCallback(() =>
    activeQuestions.map((q, i) => ({
      questionId: q.id,
      status: answers[i] === 'correct'
        ? 'correct'
        : answers[i] === 'wrong'
        ? 'incorrect'
        : 'unanswered',
  ...
  // Yakunlash button: check unanswered → toast+navigate, or show modal
  const handleYakunlash = useCallback(() => {
    const unansweredIdx = answers
      .map((a, i) => (a === null || a === 'unanswered' ? i : -1))
      .filter((i) => i >= 0)
  ...
  const handleFinishFromModal = useCallback(() => {
    setShowResults(false)
    navigate('/')
  }, [navigate])

  const handleGoToQuestion = useCallback((i) => {
    setShowResults(false)
    setCurrent(i)
  }, [])
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#0d1117]">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-[#30363d]">
        <button onClick={() => navigate(-1)} className="text-[#8b949e] hover:text-white text-lg px-1">
          ←
        </button>
        ...
        <div className="flex items-center gap-3">
          {isFinished && (
            <button
              ...
              className="text-[#1f6feb] hover:text-blue-300"
            >
              <BarChart2 size={20} />
            </button>
          )}
          <button
            ...
            className={isSaved ? 'text-yellow-400' : 'text-[#8b949e] hover:text-white'}
          >
            <Bookmark size={20} fill={isSaved ? 'currentColor' : 'none'} />
          </button>
          <button className="text-[#8b949e] hover:text-white"><Share2 size={20} /></button>
          <button onClick={() => setShowSettings(true)} className="text-[#8b949e] hover:text-white">
            <Settings size={20} />
          </button>
      ...
      <div className="flex gap-3 px-4 py-3 border-t border-[#30363d] bg-[#0d1117]">
        ...
        {(isLast || allAnswered) ? (
          ...
          <button
            onClick={selected ? goNext : undefined}
      ...
      {showResults && (
        <ResultsModal
          ...
          onRetry={handleRetry}
          onFinish={handleFinishFromModal}
