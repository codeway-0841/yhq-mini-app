/**
 * Octagon PvP page.
 *
 * States: idle → searching → matched → in_round → match_end
 *
 * The server no longer sends all questionIds upfront (security fix).
 * Current question is revealed per-round via the `question` WS message.
 */

import { useEffect, useReducer, useCallback, useRef } from 'react'
import { useNavigate } from 'react-router-dom'
import { Sword, X, Loader2 } from 'lucide-react'
import { useAppStore }    from '../store/useAppStore'
import { useT }           from '../lib/i18n'
import { questions }      from '../data/questions'
import {
  getOctagonSocket,
  type OctagonMsg,
} from '../lib/octagon-ws'

// ── State machine ──────────────────────────────────────────────────────────

type Phase = 'idle' | 'searching' | 'matched' | 'in_round' | 'match_end'

interface State {
  phase:             Phase
  matchId:           string | null
  opponentName:      string | null
  roundCount:        number           // total rounds in this match
  roundIndex:        number           // current 0-based round
  currentQuestionId: number | null    // set by 'question' server message
  yourScore:         number
  oppScore:          number
  selected:          string | null    // optionId chosen this round
  ackCorrect:        boolean | null   // set after answer_ack
  oppAnswered:       boolean
  result:            'win' | 'lose' | 'draw' | null
  toastMsg:          string | null
}

type Action =
  | { type: 'SEARCHING' }
  | { type: 'CANCEL' }
  | { type: 'MATCHED';      matchId: string; opponentName: string; roundCount: number }
  | { type: 'START_ROUND';  index: number; questionId: number }
  | { type: 'SELECT';       optionId: string }
  | { type: 'ANSWER_ACK';   correct: boolean }
  | { type: 'OPP_ANSWERED' }
  | { type: 'ROUND_RESULT'; yourScore: number; oppScore: number }
  | { type: 'MATCH_END';    yourScore: number; oppScore: number; result: 'win' | 'lose' | 'draw' }
  | { type: 'OPP_DISCONNECTED' }
  | { type: 'TOAST';        msg: string }
  | { type: 'CLEAR_TOAST' }

const INIT: State = {
  phase: 'idle', matchId: null, opponentName: null,
  roundCount: 0, roundIndex: 0, currentQuestionId: null,
  yourScore: 0, oppScore: 0,
  selected: null, ackCorrect: null, oppAnswered: false,
  result: null, toastMsg: null,
}

function reducer(s: State, a: Action): State {
  switch (a.type) {
    case 'SEARCHING':
      return { ...INIT, phase: 'searching' }
    case 'CANCEL':
      return { ...INIT }
    case 'MATCHED':
      return { ...s, phase: 'matched', matchId: a.matchId, opponentName: a.opponentName, roundCount: a.roundCount }
    case 'START_ROUND':
      return { ...s, phase: 'in_round', roundIndex: a.index, currentQuestionId: a.questionId, selected: null, ackCorrect: null, oppAnswered: false }
    case 'SELECT':
      return { ...s, selected: a.optionId }
    case 'ANSWER_ACK':
      return { ...s, ackCorrect: a.correct }
    case 'OPP_ANSWERED':
      return { ...s, oppAnswered: true }
    case 'ROUND_RESULT':
      return { ...s, yourScore: a.yourScore, oppScore: a.oppScore }
    case 'MATCH_END':
      return { ...s, phase: 'match_end', yourScore: a.yourScore, oppScore: a.oppScore, result: a.result }
    case 'OPP_DISCONNECTED':
      return { ...s, phase: 'match_end', result: 'win', toastMsg: "Raqib uzildi — g'alaba sizniki!" }
    case 'TOAST':
      return { ...s, toastMsg: a.msg }
    case 'CLEAR_TOAST':
      return { ...s, toastMsg: null }
    default:
      return s
  }
}

// ── Component ──────────────────────────────────────────────────────────────

const WS_URL = (import.meta.env.VITE_WS_URL as string | undefined)
  ?? `${location.protocol === 'https:' ? 'wss' : 'ws'}://${location.host}/ws/octagon`

export default function OctagonPage() {
  const navigate = useNavigate()
  const { user, settings } = useAppStore()
  const tt = useT(settings.language)
  const [s, dispatch] = useReducer(reducer, INIT)
  const toastTimer = useRef<ReturnType<typeof setTimeout> | null>(null)

  const showToast = useCallback((msg: string) => {
    dispatch({ type: 'TOAST', msg })
    if (toastTimer.current) clearTimeout(toastTimer.current)
    toastTimer.current = setTimeout(() => dispatch({ type: 'CLEAR_TOAST' }), 3000)
  }, [])

  const handleMsg = useCallback((msg: OctagonMsg) => {
    switch (msg.type) {
      case 'matched':
        dispatch({ type: 'MATCHED', matchId: msg.matchId, opponentName: msg.opponentName, roundCount: msg.roundCount })
        break
      case 'question':
        dispatch({ type: 'START_ROUND', index: msg.index, questionId: msg.questionId })
        break
      case 'answer_ack':
        dispatch({ type: 'ANSWER_ACK', correct: msg.correct })
        break
      case 'opp_answered':
        dispatch({ type: 'OPP_ANSWERED' })
        break
      case 'round_result':
        dispatch({ type: 'ROUND_RESULT', yourScore: msg.yourScore, oppScore: msg.oppScore })
        break
      case 'match_end':
        dispatch({ type: 'MATCH_END', yourScore: msg.yourScore, oppScore: msg.oppScore, result: msg.result })
        break
      case 'opp_disconnected':
        dispatch({ type: 'OPP_DISCONNECTED' })
        break
      case 'error':
        showToast(msg.message)
        if (msg.message === 'queue_timeout') dispatch({ type: 'CANCEL' })
        break
    }
  }, [showToast])

  useEffect(() => {
    const sock = getOctagonSocket(WS_URL)
    return sock.on(handleMsg)
  }, [handleMsg])

  // Leave queue on unmount if still searching
  useEffect(() => {
    return () => {
      if (s.phase === 'searching' && user?.id) {
        try {
          getOctagonSocket(WS_URL).send({ type: 'leave_queue', userId: user.id })
        } catch { /* socket may already be closed */ }
      }
    }
  }, [s.phase, user?.id])

  const joinQueue = useCallback(() => {
    if (!user) return
    dispatch({ type: 'SEARCHING' })
    try {
      getOctagonSocket(WS_URL).send({ type: 'join_queue', userId: user.id, name: user.firstName })
    } catch {
      dispatch({ type: 'CANCEL' })
      showToast("Ulanishda xato. Qayta urinib ko'ring.")
    }
  }, [user, showToast])

  const leaveQueue = useCallback(() => {
    if (user) {
      try { getOctagonSocket(WS_URL).send({ type: 'leave_queue', userId: user.id }) }
      catch { /* ok */ }
    }
    dispatch({ type: 'CANCEL' })
  }, [user])

  const sendAnswer = useCallback((optionId: string) => {
    if (!s.matchId || s.selected) return
    dispatch({ type: 'SELECT', optionId })
    try {
      getOctagonSocket(WS_URL).send({
        type: 'answer', matchId: s.matchId, index: s.roundIndex, optionId,
      })
    } catch {
      showToast("Javob yuborilmadi. Aloqa yo'q.")
    }
  }, [s.matchId, s.selected, s.roundIndex, showToast])

  // Current question looked up from local data by ID (revealed per-round by server)
  const currentQ = s.currentQuestionId !== null
    ? questions.find((q) => q.id === s.currentQuestionId) ?? null
    : null

  // ── Render ───────────────────────────────────────────────────────────────

  return (
    <div className="flex flex-col min-h-screen bg-[#0d1117]">
      {/* Header */}
      <div className="flex items-center justify-between px-4 py-3 border-b border-[#30363d]">
        <button onClick={() => navigate(-1)} className="text-[#8b949e] p-1">
          <X size={20} />
        </button>
        <div className="flex items-center gap-2">
          <Sword size={16} className="text-purple-400" />
          <span className="text-sm font-bold">{tt('octagonTitle')}</span>
        </div>
        {s.phase === 'in_round' ? (
          <div className="flex gap-1 text-xs text-[#8b949e]">
            <span className="text-green-400 font-bold">{s.yourScore}</span>
            <span>:</span>
            <span className="text-red-400 font-bold">{s.oppScore}</span>
          </div>
        ) : (
          <div className="w-8" />
        )}
      </div>

      {/* Toast */}
      {s.toastMsg && (
        <div className="mx-4 mt-2 bg-orange-900/60 border border-orange-500/50 text-orange-200 text-xs font-semibold px-3 py-2 rounded-xl text-center">
          {s.toastMsg}
        </div>
      )}

      {/* Body */}
      <div className="flex-1 flex flex-col items-center justify-center px-4">

        {/* IDLE */}
        {s.phase === 'idle' && (
          <div className="flex flex-col items-center gap-5 text-center">
            <Sword size={56} className="text-purple-400 opacity-80" />
            <div>
              <h2 className="text-xl font-black mb-1">{tt('octagonTitle')}</h2>
              <p className="text-sm text-[#8b949e]">Haqiqiy vaqtda raqib bilan bellashuv</p>
            </div>
            <button
              onClick={joinQueue}
              className="bg-purple-600 text-white font-bold px-8 py-3.5 rounded-xl text-base"
            >
              {tt('findOpponent')}
            </button>
          </div>
        )}

        {/* SEARCHING */}
        {s.phase === 'searching' && (
          <div className="flex flex-col items-center gap-5 text-center">
            <Loader2 size={40} className="text-purple-400 animate-spin" />
            <p className="text-base font-bold">{tt('searching')}</p>
            <p className="text-xs text-[#8b949e]">Raqib qidirilmoqda...</p>
            <button
              onClick={leaveQueue}
              className="text-sm text-[#8b949e] border border-[#30363d] px-5 py-2.5 rounded-xl"
            >
              {tt('cancel')}
            </button>
          </div>
        )}

        {/* MATCHED — waiting for first question */}
        {s.phase === 'matched' && (
          <div className="flex flex-col items-center gap-4 text-center">
            <Sword size={36} className="text-purple-400" />
            <p className="text-lg font-black">VS</p>
            <p className="text-base font-bold text-purple-300">{s.opponentName}</p>
            <p className="text-xs text-[#8b949e] animate-pulse">Tayyor bo'ling...</p>
          </div>
        )}

        {/* IN ROUND */}
        {s.phase === 'in_round' && currentQ && (
          <div className="w-full max-w-md">
            <p className="text-xs text-[#8b949e] mb-1 text-center">
              {tt('round')} {s.roundIndex + 1} / {s.roundCount}
              {s.oppAnswered && !s.selected && (
                <span className="ml-2 text-orange-400">• Raqib javob berdi</span>
              )}
            </p>
            <p className="text-base font-semibold text-center mb-5 leading-snug">
              {currentQ.text}
            </p>
            {currentQ.options.map((opt) => {
              const answered   = !!s.selected
              const isSelected = s.selected === opt.id
              // Reveal correct answer after ack arrives (ackCorrect !== null)
              const showCorrect = answered && s.ackCorrect !== null && opt.id === currentQ.correct
              const style =
                !answered      ? 'bg-[#161b22] border-[#30363d] text-[#e6edf3]' :
                showCorrect    ? 'bg-green-900/40 border-green-600 text-white' :
                isSelected && s.ackCorrect  ? 'bg-green-900/60 border-green-500 text-white' :
                isSelected && !s.ackCorrect ? 'bg-red-900/60   border-red-500   text-white' :
                                              'bg-[#161b22] border-[#30363d] text-[#8b949e]'
              return (
                <button
                  key={opt.id}
                  disabled={answered}
                  onClick={() => sendAnswer(opt.id)}
                  className={`w-full text-left rounded-xl border p-3.5 mb-2 transition-all active:scale-[0.98] ${style}`}
                >
                  <div className="flex items-center gap-3">
                    <span className="w-7 h-7 rounded-full border border-current/30 flex items-center justify-center text-xs font-bold opacity-60 flex-shrink-0">
                      {opt.id}
                    </span>
                    <span className="text-sm">{opt.text}</span>
                  </div>
                </button>
              )
            })}
          </div>
        )}

        {/* IN ROUND — question not yet received */}
        {s.phase === 'in_round' && !currentQ && (
          <Loader2 size={28} className="text-purple-400 animate-spin" />
        )}

        {/* MATCH END */}
        {s.phase === 'match_end' && (
          <div className="flex flex-col items-center gap-5 text-center">
            <span className="text-6xl">
              {s.result === 'win' ? '🏆' : s.result === 'lose' ? '😔' : '🤝'}
            </span>
            <h2 className="text-2xl font-black">
              {s.result === 'win' ? tt('youWon') : s.result === 'lose' ? tt('youLost') : tt('draw')}
            </h2>
            <div className="flex gap-6 font-bold">
              <div className="text-center">
                <p className="text-green-400 text-3xl">{s.yourScore}</p>
                <p className="text-xs text-[#8b949e] mt-1">Siz</p>
              </div>
              <div className="text-[#30363d] text-3xl self-center">:</div>
              <div className="text-center">
                <p className="text-red-400 text-3xl">{s.oppScore}</p>
                <p className="text-xs text-[#8b949e] mt-1">{s.opponentName}</p>
              </div>
            </div>
            <div className="flex gap-3 w-full max-w-xs">
              <button
                onClick={() => dispatch({ type: 'CANCEL' })}
                className="flex-1 py-3 rounded-xl bg-[#21262d] text-sm font-semibold"
              >
                Chiqish
              </button>
              <button
                onClick={joinQueue}
                className="flex-[2] py-3 rounded-xl bg-purple-600 text-white font-bold"
              >
                Qayta o'ynash
              </button>
            </div>
          </div>
        )}

      </div>
    </div>
  )
}
