...
- 🟡 LOW batch: Gemini kaliti `x-goog-api-key` headerga (2 joy), admin statistika Asia/Tashkent, `pdf-parse` o'chirilgan, `api/*.js` untrack+gitignore, `standalone.ts`→`index.ts` birlashtirilgan (`/health` alias app.ts'da, render.yaml yangilangan), `loginPendingCodes` TTL, `__APP_VERSION__` reproducible, analytics/login/link kodlarga cron cleanup, TestPage L11 updater side-effect'siz, OAuth stub'lar endi `available:false` 501. *Ochiq:* email enumeration, ~59 hardcoded rang, modal a11y, i18n qoldiqlar, xlsx/PDF import
- ✅ FIXED (texnik qarz partiyasi): **P2 jsonb** — `progress_questions` jadvali (migratsiya 0043/0044, backfill idempotentligi isbotlangan; O(1) upsert, gate index-EXISTS; client kontrakti o'zgarmagan), **repository pattern** — `admin.repository`/`analytics.repository`/`cron.repository` (router'lar endi sof orchestration; admin DELETE bitta CTE), **graceful shutdown** registry (`utils/shutdown.ts`), **dup index** migratsiya 0045, **App.tsx remount olib tashlandi** (scroll+animation `pageRef` orqali), **ESLint** flat config + CI (1 ta haqiqiy hooks violation tuzatildi), **repository coverage** — `repo-coverage.test.ts` (9 test: race/atomic/hash)
