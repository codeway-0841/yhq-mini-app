...
13. **Safe-area (tepa):** yangi `fixed`/`sticky` TEPA element yoki fullscreen overlay header'i — FAQAT `var(--safe-top)` / `var(--safe-top-body)` / `.safe-top` (index.css SSOT); literal `top-N` (top-0, top-4...) TAQIQLANADI — APK edge-to-edge (targetSdk 36) va TG fullscreen'da status bar/TG tugmalari ostida qoladi. Buzilishni `tests/unit/config/safe-area.test.ts` ushlaydi (2026-09-01: o'lik `.safe-top` class incident). **SCROLLPORT TUZOG'I (2026-09-01):** haqiqiy scroll'ni DOCUMENT bajaradi — App.tsx Layout/route-page konteynerlarida `overflow-y-auto`/`overflow-hidden` TAQIQLANADI: scroll bo'lmasa ham sticky uchun containing block bo'lib qoladi → sticky header viewport'ga yopishmay "suzadi" (Admin incident, Playwright-tasdiq); ruxsat FAQAT `overflow-x-clip` (scrollport yaratmaydi). Sticky tepa bar fonini OPAQUE qiling (shaffof `/90` ostidan kontent qanab ko'rinadi). Fullscreen/APK'da tepa nafas = inset+16px (`--safe-top-body`). **APK (native) ZIXIROQ:** `body[data-platform=native]` override — inset+**8px** (TG tugmalari yo'q, faqat status bar; 2026-09-01 osonprava taqqoslash). **Custom scrollbar FAQAT desktop:** `::-webkit-scrollbar` qoidalari `@media (hover:hover) and (pointer:fine)` ICHIDA — touch WebView'da native auto-hide (2026-09-01 "scroll ko'rinadi" incident).
13a. **Safe-area (PASTKI, 2026-09-01 audit):** `--safe-bottom` = `max(env(safe-area-inset-bottom), var(--tg-content-safe-area-inset-bottom))` (index.css SSOT) — Android TG fullscreen'da `env()`=0 qaytarishi mumkin, TG var'i ZAXIRA sifatida SHART (ikkalasini max() bilan oling, faqat env()'ga tayanmang!). Yangi `fixed` PASTKI element — `bottom-[calc(<bazaviy>+var(--safe-bottom,0px))]` (literal `bottom-N` taqiqlangan, test ushlaydi); pastga yopishadigan sheet'lar uchun MARKAZIY lift — `DialogOverlay` (position='bottom') konteyneridagi `pb-[var(--safe-bottom)]` (qo'lda sheet'ga yozish KERAK EMAS, ikkilanib qoladi); sahifa kontentining scroll-oxiri himoyasi — App.tsx route-page `pb-[calc(1rem+var(--safe-bottom))]` (markaziy). `.safe-bottom` class'i SOF inset (bazaviy nafasni o'z ichiga olmaydi, pb-*'ni yengadi — birga yozmang). Oddiy rejimda --safe-bottom=0 → ko'rinish O'ZGARMAYDI.
