...
7f. **APK env prioriteti + ESKI LOYIHA tuzog'i (2026-08-31 APK login incident'ining HAQIQIY sababi):** `.env.production.local` production build'da `.env.local`'dan YUQORI prioritet (Vite mode-specific tartib) — APK build'da shu fayl g'alaba qozonadi. `yhq-mini-app.vercel.app` — domen split'dan (2026-08-30) qolgan **ESKI ALOHIDA Vercel loyihasi**: yangi deploy'lar u yerga KETMAYDI, lekin DB umumiy — chalg'ituvchi "yarim ishlaydi" holati (kod yaratish + bot tasdiqlash ishlaydi, yangi fix'lar yo'q). APK TG-login spinner'ining ildiz sababi shu edi: `.env.production.local`'dagi eski `VITE_API_BASE_URL=https://yhq-mini-app.vercel.app/api` — eski loyihada `X-Login-Code` CORS'da yo'q edi. FIX: APK build env'da FAQAT `https://kivvi.uz/api` (apex). Eski loyihani Vercel dashboard'da o'chirish/disable qilish tavsiya etiladi.
