...
8b. **Coins iqtisodiyoti (#40):** balans FAQAT server'da (`user_coins` + `coin_transactions` ledger UNIQUE(user,reason,ref) + `user_items` egalik, users.avatar_frame). Mint FAQAT `progress.repository.recordAnswer` CTE'dagi `coin_award` orqali (gate'dan o'tgan TO'G'RI javob: anti-farm/kredit/replay coin'ni ham to'xtatadi); debit FAQAT atomik purchase CTE'lar (coins.repository) — client narxi/balansni HECH QACHON yozmaydi. Katalog SSOT: `shared/shop-items.ts` (tema/premium-days/ramka — narx server-side shu yerdan), `shared/daily-tasks.ts` (vazifalar: claim CTE `taskId:<date>` UNIQUE → 1/kun), `shared/merch-items.ts` (merch: 1-per-user + stock guard + cancel'da atomik `merch_refund`). Desync: `tests/unit/config/shop-items.test.ts` / `daily-tasks.test.ts` / `merch-items.test.ts`. Yangi do'kon buyumi: config'ga yozuv + kerak bo'lsa i18n (UZ+RU) — `resolveAccent(id, isPremium, owned)` gating (free+unowned coin-tema ochilmaydi). Endpoint'lar promo-style: `req.userId` (`requireAuth`-ekvivalent ichki guard), per-endpoint `dbRateLimit` bucket'lar (`coins:purchase|equip|claim|merch`).
